...
(function($){
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showPrev){
			if(args.current > 1){
				obj.append('<a href="javascript:;" class="prevPage"><span class="u-ico"><</span>上一页</a>');
			}else{
				obj.remove('.prevPage');
				obj.append('<span class="disabled"><span class="u-ico"><</span>上一页</span>');
			}
		}
		//中间页码
		if(args.current != 1){
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				obj.remove('.nextPage');
				obj.append('<span class="disabled">下一页<span class="u-ico">></span></span>');
			}else{
				obj.append('<a href="javascript:;" class="nextPage">下一页<span class="u-ico">></span></a>');
			}
		}
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.find('input').css({'outline':'none','border':'1px solid #ddd','height':s.height-2+'px','line-height':s.height-2+'px','width':'30px',margin: '0 5px',textAlign: 'center'});
	...
	Page.prototype.bindEvent=function(){
		...
		var _this=this;
			...
			obj.on("click","a.tcdNumber",function(){
				$('.m-co-option-right .u-nowPage').text(parseInt($(this).text()))
				_this.args.current = parseInt($(this).text());
				_this.fillHtml();
			});
			//上一页
			obj.on("click","a.prevPage",function(){
				$('.m-co-option-right .u-nowPage').text(parseInt(obj.children("span.current").text())-1)
			...
			obj.on("click","a.turndown",function(){
				...
				if (page) {
					...
					$('.m-co-option-right .u-nowPage').text(Number(obj.children("span.countYe").children('input').val()))
					_this.fillHtml();
				}
			});
		}
